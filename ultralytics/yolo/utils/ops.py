...
from ultralytics.yolo.utils import LOGGER, DEFAULT_CFG
...
def scale_points(img1_shape, points, img0_shape, ratio_pad=None):
    ...
    points[..., ::3] -= pad[0]  # x padding
    points[..., 1::3] -= pad[1]  # y padding
    points[..., ::3]  /= gain
    points[..., 1::3] /= gain
    return points
...
def non_max_suppression(
    ...
    nc = prediction.shape[1] - nm - 4 - 3 * kpt_num  # number of classes
    ...
    t = time.time()
    output = [torch.zeros((0, 6 + nm + 3 * kpt_num), device=prediction.device)] * bs
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        box, cls, point, mask = x.split((4, nc, 3*kpt_num, nm), 1)
        box = xywh2xyxy(box)  # center_x, center_y, width, height) to (x1, y1, x2, y2)
        if multi_label:
            i, j = (cls > conf_thres).nonzero(as_tuple=False).T
            x = torch.cat((box[i], x[i, 4 + j, None], j[:, None].float(), point[i]), 1)
        else:  # best class only
            conf, j = cls.max(1, keepdim=True)
            x = torch.cat((box, conf, j.float(), point), 1)[conf.view(-1) > conf_thres]
