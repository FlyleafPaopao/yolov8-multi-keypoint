...
class RandomPerspective:
    ...
    def apply_keypoints(self, keypoints, M):
        ...
        if n == 0:
            return keypoints
        new_keypoints = np.ones((n * self.kpt_num, 3))
        new_keypoints[:, :2] = keypoints.reshape(n * self.kpt_num, 2)  # num_kpt is hardcoded to 17
        new_keypoints = new_keypoints @ M.T  # transform
        new_keypoints = (new_keypoints[:, :2] / new_keypoints[:, 2:3]).reshape(n, self.kpt_num * 2)  # perspective rescale or affine
        new_keypoints[keypoints.reshape(-1, self.kpt_num * 2) == 0] = 0
        x_kpts = new_keypoints[:, list(range(0, self.kpt_num * 2, 2))]
        y_kpts = new_keypoints[:, list(range(1, self.kpt_num * 2, 2))]
        ...
        new_keypoints[:, list(range(0, self.kpt_num * 2, 2))] = x_kpts
        new_keypoints[:, list(range(1, self.kpt_num * 2, 2))] = y_kpts
        return new_keypoints.reshape(n, self.kpt_num, 2)
...
class Format:
    ...
    def __call__(self, labels):
        ...
        if self.return_keypoint:
            labels["keypoints"] = torch.from_numpy(instances.keypoints) if nl else torch.zeros((nl, self.kpt_num, 2))
