...
def verify_image_label(args):
    ...
    im_file, lb_file, prefix, keypoint, keypoints_number = args
    ...
    try:
        ...
        if os.path.isfile(lb_file):
            ...
            if nl:
                if keypoint:
                    assert lb.shape[1] == 5 + 2 * keypoints_number, "labels require 5 + 2 * keypoints_number columns each"
                    assert (lb[:, 5::2] <= 1).all(), "non-normalized or out of bounds coordinate labels"
                    assert (lb[:, 6::2] <= 1).all(), "non-normalized or out of bounds coordinate labels"
                else:
                ...
                ne = 1  # label empty
                lb = np.zeros((0, 5 + 2 * keypoints_number), dtype=np.float32) if keypoint else np.zeros((0, 5), dtype=np.float32)
        else:
            nm = 1  # label missing
            lb = np.zeros((0, 5 + 2 * keypoints_number), dtype=np.float32) if keypoint else np.zeros((0, 5), dtype=np.float32)
        if keypoint:
            keypoints = lb[:, 5:].reshape(-1, keypoints_number, 2)
        lb = lb[:, :]
